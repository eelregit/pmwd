...
We normalize the gradients by the standard deviation of each parameter
across all samples of $\vvartheta^{(i)}$, and get $\vsigma \cdot
\partial f / \partial \vvartheta^{(i)}$.
The final sensitivity is given by
\begin{equation}
\frac{1}{N} \sum_{i=1}^{N} \Bigl(\vsigma \cdot \frac{\partial f}{
                                 \partial \vvartheta^{(i)}}\Bigr)^2 .
\end{equation}

...
$\vo$ along with the state $\vz$
\begin{align}
\vz_{i+1} &= \vz_i + \vI_i(\vz_i, \vtheta), \\
\vo_{i+1} &= \vo_i + \vO_{i+1}(\vz_{i+1}, \vtheta),
\end{align}
where $i=0, \cdots, n-1$ and the observe function $\vO$ depends only on
the state but not the observable itself, which is consistent with the
case in this work.
The observable is initialized by observing the initial state, i.e.
...
\end{align}
The adjoint equations could be derived
\begin{align}
\vmu_{i-1} &= \vmu_{i}, \\
...
\end{align}
where $i = n, \cdots, 1$.
The initial conditions of the adjoint variables are
\begin{align}

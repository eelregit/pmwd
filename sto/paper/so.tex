...
% \end{tikzpicture}
% }
\caption{\subref{fig:loop} shows the flow of the state $\vz$ and the
...
\vo_i &= \vo_{i-1} + \vO_i(\vz_i, \vtheta), \label{obsvbl_fwd}
\end{align}
where $\vI$ and $\vO$ are the integration and observation functions
respectively, $i=1, \cdots, n$, and we observe the new state every
step after it is updated from the integration function.
...
leapfrog integrator, each integration-observation step could be

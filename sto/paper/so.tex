...
settings.
For gravitational force, we use the pure FMM solver with multipole order
$p=5$, a geometric (Barnes-Hut) opening criterion with opening angle
$\theta=0.4$, hierarchical time integration, and extra-high Ewald
summation accuracy for the periodic boundary.
For time integration, we set the maximum timestep size to 0.001 and the
error tolerance to 0.005.
The simulation runs in double precision with randomized domain centers.
...
Small numerical differences in initial conditions could become more significant
with the $N$-body evolution.
Therefore, besides the output snapshots, the initial conditions used in the
\GADGET\ simulations are also loaded from file for \pmwd\ during training to
make sure the initial conditions are consistent for both the training data and
the \pmwd\ model.
...
For both the $f$ and $g$ in the SO force equation \eqref{force_so},
their input features consist of tabular data, therefore we adopt neural
networks with fully connected layers, aka multilayer perceptrons (MLPs)
to learn the nonlinear functions.
...
time integration step, can be treated just similar to other

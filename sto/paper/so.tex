...
The inputs to both $f$ and $g$ in the SO force equation \eqref{force_so} are
tabular features, we therefore adopt multilayer perceptrons (MLPs) to learn
these nonlinear functions.
Both $f$ and $g$ networks have 5 hidden layers, with 64 and 128 nodes per
layer respectively.
The $g$ network is wider given its more complex, higher-dimensional input
features.
...
can be trained using gradient-based optimization methods as in standard
deep learning tasks.
The parameters of the NNs, which enter the force evaluation at every time step,
can be treated just like any other cosmological parameters.

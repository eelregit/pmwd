...
\begin{tikzpicture}[node style]
  \node (ic) {LPT};
  \node (integ_1) [right=of ic] {$I_1(\vvartheta)$}
        edge [from] (ic);
  \node (obs_1) [below=of integ_1] {$O_1$}
        edge [from] (integ_1);
  \node (integ_1i) [right=of integ_1] {$\cdots$}
        edge [from] (integ_1);
  \node (obs_1i) [below=of integ_1i] {$\cdots$}
        edge [from] (integ_1i)
        edge [ofrom] (obs_1);
  \node (integ_i) [right=of integ_1i] {$I_i(\vvartheta)$}
        edge [from] (integ_1i);
  \node (obs_i) [below=of integ_i] {$O_i$}
        edge [from] (integ_i)
        edge [ofrom] (obs_1i);
  \node (integ_in) [right=of integ_i] {$\cdots$}
        edge [from] (integ_i);
  \node (obs_in) [below=of integ_in] {$\cdots$}
        edge [from] (integ_in)
        edge [ofrom] (obs_i);
  \node (integ_n) [right=of integ_in] {$I_n(\vvartheta)$}
        edge [from] (integ_in);
  \node (obs_n) [below=of integ_n] {$O_n$}
        edge [from] (integ_n)
        edge [ofrom] (obs_in);
  \node (obj) [right=of obs_n] {Obj}
        edge [ofrom] (obs_n);
  \node (box) [box style, fit=(integ_1) (obs_n)] {};
  \node [box style, inner sep=0em, dotted, fit=(integ_i) (obs_i)] {};
...
\begin{tikzpicture}[node style, node distance=2em and 1.5em]
  ...
  \node (drift) [right=of kick_a] {$D_{i-1}^i$}
        edge [from] (kick_a);
  \node (force) [right=of drift] {$F_i(\vvartheta)$}
        edge [from] (drift);
  ...
  \node (obs_ip1) [below=of integ_ip1, minimum width=0em] {}
        edge [ofrom] (obs);
  \node (box) [box style, dotted, fit=(kick_a) (kick_b) (obs)] {};
  \useasboundingbox ($ (box.south) - (0, 1em) $) --
                    ($ (box.north) + (0, 1em) $);
\end{tikzpicture}
}
\caption{The cosmological N-body forward simulation model.
The solid arrows denote the flow of particles' state $(\vx, \vp)$, which
are initialized with LPT and updated in the N-body
integration-observation loop.
As shown in \eqref{force_so}, our spatial optimization neural networks
with parameters $\vvartheta$ are embedded in the Poisson equation of the
force operator $F_i$, which is part of the integration operator $I_i$.
The dashed arrows denote the flow of the observable $\vo$, which in the
end enters the objective function.}
\end{figure*}
...
\vp/(a^3H)$, where $H$ is the Hubble parameter.} $\vv := \d\vs/\d a$ at
two consecutive time steps $a_i$ and $a_{i+1}$, we get the snapshot of
particles in between at $a$ using the cubic Hermite interpolation.
...
The interpolated snapshots at different times constitute our observable,
where the VJPs of the interpolation functions are computed by AD as an
intermediate step during the computation of $\p\vo / \p\vx$
and $\p\vo / \p\vp$ in the adjoint equations
\eqref{adjoint_obs} and \eqref{adjoint_init}.


% \vspace{1em}
\subsection{Adjoint Equations with Observables}
Following \cite{Li2022a}, here we derive the adjoint equations with
observables that are explicitly formulated for this work.
For the forward modeling, we consider the evolution of the observable
$\vo$ along with the state $\vz$ following
\begin{align}
\vz_{i+1} &= \vz_i + \vI_i(\vz_i, \vtheta), \\
\vo_{i+1} &= \vo_i + \vO_{i+1}(\vz_{i+1}, \vtheta), \label{obsvbl_fwd}
\end{align}
where $i=0, \cdots, n-1$, and the observable is initialized by observing
the initial state, i.e. $\vo_0 = \vO_0(\vz_0, \vtheta)$.
We take an observe function $\vO$ that depends only on the state but not
the observable itself, which is consistent with the case in this work
where the state of particles at each step contributes independently to
the observable.
The derivation below remains much the same with very few modifications
for a more general observe function.

Assuming an objective function of the final observable, the Lagrangian
reads
\begin{align}
\cL = \cJ(\vo_n)
&- \sum_{i=0}^{n-1} \vlambda_{i+1}
  \cdot \bigl[ \vz_{i+1} - \vz_i - \vI_i(\vz_i, \vtheta) \bigr] \nonumber\\
&- \sum_{i=0}^{n-1} \vmu_{i+1}
  \cdot \bigl[ \vo_{i+1} - \vo_i - \vO_{i+1}(\vz_{i+1}, \vtheta) \bigr],
\end{align}
...
\begin{align}
\vmu_{i-1} &= \vmu_{i}, \\
\vlambda_{i-1} &= \vlambda_{i}
  + \vlambda_{i}\cdot\frac{\p\vI_{i-1}}{\p\vz_{i-1}}
  + \vmu_{i-1}\cdot\frac{\p\vO_{i-1}}{\p\vz_{i-1}},
\end{align}
for $i = n, \cdots, 1$ and the initial conditions of the adjoint
variables are
\begin{align}
...
the forward modeling, where neither the integrate ($\vI$) nor the
observe ($\vO$) function depends on the observable ($\vo$).
Finally, the objective gradients could be accumulated through
\begin{align}
\frac{d\cJ}{d\vtheta} =
\vmu_0\cdot\frac{\p\vo_0}{\p\vtheta}
+ \left(\vlambda_0 - \vmu_0\cdot\frac{\p\vO_0}{\p\vz_0}\right)\cdot\frac{\p\vz_0}{\p\vtheta}
+ \sum_{i=1}^{n}\left[\vlambda_{i}\cdot\frac{\p\vI_{i-1}}{\p\vtheta}
+ \vmu_{i}\cdot\frac{\p\vO_{i}}{\p\vtheta}\right].
\end{align}
...
YZ was supported by the National Natural Science Foundation of China
under Grant No. 12503006, and the China Postdoctoral Science Foundation
under Grant No. 2023M731831.

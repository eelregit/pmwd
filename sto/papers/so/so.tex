...
To improve the accuracy of the PM solver, we introduce force sharpening for
spatial optimization (SO) by modifying the gravitational force equation
\eqref{force} to
%
\begin{equation}
F_i(\vk) = i \frac32 \Omegam H_0^2 \,\frac{k_i}{k^2}\,
                    f(k_i; \vvartheta) g(\vk; \vvartheta) \delta(\vk),
\label{force_so}
\end{equation}
%
where $f$ and $g$ are nonlinear functions of the wavevector components and
relevant features $\vvartheta$ (detailed in \autoref{sec:fsnet}), which are
approximated with NNs.
$k_i$ is a component of $\vk$, with $i\in\{1,2,3\}$ denoting the 3
principal axes of the simulation box.
Gravity obeys the $\mathrm{O}(3)$ rotational symmetry, which is reduced by the
Cartesian mesh of the PM solver to the $\mathrm{O_h}$ discrete symmetry with
48 elements involving permutations and reversals of the 3 principal axes of the
simulation box.
The PM evaluation of a given force component $F_i$ further breaks the
permutation symmetry along axis $i$, while the remaining two axes remain
permutationally symmetric.
Therefore, we implement $f$ as an even function of its wavevector argument
$k_i$, and $g$ as an even and permutation-equivariant function of all three
wavevector components.
Together, $fg$ provides the most general kernel modification consistent with the
PM solver's symmetries.
Enforcing $f$ and $g$ to be even functions in Fourier space corresponds to a
real and symmetric kernel in configuration space, consistent with the physical
force.
We take the absolute value of the wavevectors for the evenness, and sort
the 3 wavevector components to achieve the permutation equivariance.

We approximate $f$ and $g$ with multilayer perceptrons (MLPs), with
the network architecture and training details described in
\autoref{sec:fsnet}.
As we will show, these networks learn to sharpen the PM force kernel,
significantly improving the accuracy of the simulated density fields on small
scales.
...
Each simulation evolves $128^3$ particles, saving snapshots linearly spaced in
scale factor.
We refer to this training dataset as \GSDATA.
...
To ensure our spatial optimization is robust across different scenarios, the
\GSDATA\ dataset encompasses a broad variation in box sizes (from 25.6 to 2560
Mpc$/h$) and samples a wide range of cosmological parameters and simulation
configurations (see \autoref{tab:param} for the full list).
Detailed parameter distributions, configurations, and the generation procedure
are provided in \autoref{sec:conf_data}.
...
\begin{figure*}
  \centering
  \includegraphics[width=.98\columnwidth]{dens_zoom.pdf}
  \caption{Projected dark matter density fields at $a\simeq 1$ simulated with
  original PM (\textit{left}), \GADGET\ (\textit{middle}), and PM with spatial
  optimization (\textit{right}).
  These three simulations are run with the same cosmological parameters and
  initial conditions at $a=1/16$.
  \YZ{Add more details on the box size, zoom-in window size, and slab thickness
...
density field (\autoref{fig:dens_zoom}) on small scales.
...
To quantify the agreement between two fields $\hat f$ and $f$, we use
the per-mode transfer function (TF) $T$ \eqref{T} and correlation
coefficient (CC) $r$ \eqref{r}, both defined in \autoref{sec:metrics}.
...
Although the number of time steps are fixed in this work, we emphasize that our
spatial optimization also applies for different numbers of time steps.
This will be explored, together with the optimal choice of the time variable, in
a follow-up work on temporal optimization.
...
\begin{tikzpicture}[node style]
  ...
  \node (o_0) [below=of z_0] {$\vo_0$}
        edge [obs] (z_0);
  \node (z_dots1) [right=of z_0] {$\cdots$}
        edge [integ] (z_0);
  \node (o_dots1) [below=of z_dots1] {$\cdots$}
        edge [obs] (z_dots1)
        edge [from] (o_0);
  \node (z_im1) [right=of z_dots1] {$\vz_{i-1}$}
        edge [integ] (z_dots1);
  \node (o_im1) [below=of z_im1] {$\vo_{i-1}$}
        edge [obs] (z_im1)
        edge [from] (o_dots1);
  \node (z_i) [right=of z_im1] {$\vz_i$}
        edge [integ] (z_im1);
...
\begin{tikzpicture}[node style]
  ...
  \node (o_right) [right=of o_1] {}
        edge [from] (o_1);
  \node (box) [box style, dashed, inner xsep=1.5em, fit=(z_0) (z_1) (o_0) (o_1)] {};
  \useasboundingbox ($ (box.south) - (0, 1em) $) --
                    ($ (box.north) + (0, 1em) $);
\end{tikzpicture}
}
...
simulation.
\subref{fig:step} shows explicitly a zoom-in view of one integration-observation
step for the leapfrog integrator.
The solid arrows denote the integration of the state, the dashed arrows
denote the observation of the state accumulated into the observable, and the
dotted arrows are simply the forward pass of the previous observable.}
\end{figure*}
...
work.
Following \citet{Li2022a}, we denote the particles' state with $\vz =
...
the state but not on the observable itself, which is consistent with the
...
\autoref{sec:snapobs}.
The derivation below remains largely unchanged with minor modifications for
...
\label{KDKO_adj_init}
\end{equation}
The objective gradient now reads
\begin{align}
\frac{\d\cJ}{\d\vtheta} =\
...
in terms of the scale factor $a$, while the state of particles at any
intermediate time can be obtained using interpolation.
Given the particle displacements $\vs$ and velocities $\vv \triangleq
\d\vs/\d a$ at two consecutive time steps $a_i$ and $a_{i+1}$, we compute
the snapshot of particles at a target time $a \in [a_i, a_{i+1}]$ using
...
\d\vs(a) / \d a$, which can be converted back to the canonical momenta
$\vp(a) = a^3 H(a) \vv(a)$.
The positions of particles are related to their displacements $\vs$
through $\vx = \vq + \vs$, where $\vq$ are the initial Lagrangian
positions.
Thus the full state vector of the interpolated snapshot reads
$\vz(a) = (\vx(a), \vp(a))^\intercal$.
Notice that the cubic Hermite interpolation is a linear combination of
the particles' states at two consecutive time steps, meaning each state
contributes independently to the interpolated snapshot.
This additive property is consistent with our formulation in \eqref{IO_fwd},
where the observation function at a given step does not depend on the
observable from previous steps.
Consequently, the contributions from the two bounds of the interpolation
interval can be naturally evaluated and accumulated at their respective time steps.
A series of interpolated snapshots at given target times constitutes our
observable for the objective function.
For the observation at each time step, we first determine if the
current particles' state contributes to any targeted snapshot.
If it does, we compute its partial contribution to the interpolation---acting
as either the lower or upper bound---and add this update to the
corresponding snapshot in the observable.


\vspace{1em}
\subsection{Force Sharpening Networks}
\label{sec:fsnet}

The functions $f$ and $g$ in the SO force equation \eqref{force_so} are
approximated with multilayer perceptrons (MLPs).
The input features for these networks consist of both local wavevector
components and global cosmological and simulation parameters.
Specifically, $f(k_i; \vvartheta)$ takes 10 input features, including the
wavenumber $k_i$ scaled by three physical lengths ($l_p, R_{\rm TH}, l_s$) and
7 global parameters.
The $g(\vk; \vvartheta)$ network receives 16 features, representing the full
3D wavevector $\vk$ scaled by the same physical lengths, along with the global
parameters.

Both $f$ and $g$ networks have 5 hidden layers, with 64 and 128 nodes per hidden
layer respectively, totaling approximately $8.5 \times 10^4$ trainable
parameters.
The $g$ network is wider to accommodate the higher dimensionality of the 3D
wavevector inputs and the greater functional complexity of the potential
correction compared to the single-component corrections of $f$.
We use the rectified linear unit (ReLU) activation function for the hidden
layers.
To respect the physical symmetries of the gravitational force, we enforce
the $\mathrm{O_h}$ discrete symmetry of the Cartesian mesh through the network
inputs.
The $f$ network is implemented as an even function by using the absolute value
of the wavevector component, $f(|k_i|; \vvartheta)$.
For the $g$ network, we take the absolute values of the wavevector components
and sort them, $g(\mathrm{sort}(|k_1|, |k_2|, |k_3|); \vvartheta)$, which
guarantees both axis-reversal and permutation symmetry.

The NNs are initialized such that the initial output is exactly unity,
representing a "neutral" PM simulation.
This is achieved by specific weight and bias initialization, as detailed
in \autoref{sec:training}.
Being embedded in the differentiable \pmwd\ library, the networks are trained
by backpropagating gradients from the large-scale simulation.
The NN parameters are optimized alongside any other model parameters using
gradient-based methods, with objective gradients accumulated as described in
\autoref{sec:diffsim_obs}.



\vspace{1em}
\subsection{Configurations and Data Generation}
\label{sec:conf_data}

\begin{figure*}
  \centering
  \includegraphics[width=0.9\linewidth]{sobol.pdf}
  \caption{Randomized Quasi-Monte Carlo (RQMC) configuration with a
...
\begin{table*}
  ...
  \cmidrule(lr){2-5}
  & box size in Mpc$/h$ & log-trapezoidal & [25.6, 2560) \\
  \cmidrule(lr){2-4}
  ...
  \cmidrule(lr){1-5}
  \cmidrule(lr){1-4}
  training & loss grid offset & uniform & $[0, 1)^3$ & MC \\
  \bottomrule
  \end{tabular}
\end{table*}

Training the SO model requires a diverse dataset that spans a wide range of
cosmological parameters and simulation configurations.
We generate this dataset using the \GADGET\ simulation code \citep{GADGET-4},
configured as described below.

To generate the \GSDATA\ dataset, we sample 512 data points in the 9D space of
configurations and cosmological parameters using a randomized quasi-Monte Carlo
(RQMC) method with a scrambled Sobol sequence (\autoref{fig:sobol} and
\autoref{tab:param}).
We use the \texttt{scipy.stats.qmc} package \citep{SciPy} to generate
the Sobol sequence \citep{Sobol1967}, which relies on the direction numbers
from \citet{JoeKuo2008} and Owen scrambling \citep{Owen1998}.
To ensure proper coverage, we search among 65,536 scrambling seeds to minimize
the mixture discrepancy (a uniformity measure) proposed in \citet{Zhou2013MD}.
To vary the alignment of snapshot times relative to the integration time
steps and thereby avoid systematic numerical artifacts in \pmwd, we introduce a
fractional random offset $\gamma \in (0, 1]$.
Sampling non-zero spatial curvature $\OmegaK$ enables the model to capture the
effect of long-wavelength density perturbations, which in the separate universe
picture are equivalent to a modified background curvature
\citep{LiEtAl2014, WagnerEtAl2015}.
In addition to the training set, we sample 9 extra data points to test the
interpolation accuracy of the trained model on unseen configurations and
cosmological parameters.

For each sampled configuration, we run the target \GADGET\ \citep{GADGET-4}
simulations using high force and time integration accuracy settings.
For the gravitational force, we use the pure FMM solver with a multipole order
of $p=5$, a geometric (Barnes-Hut) opening criterion with an opening angle of
$\theta=0.4$, hierarchical time integration, and extra-high Ewald
summation accuracy for the periodic boundaries.
For time integration, we set the maximum timestep size to 0.001 and the
error tolerance to 0.005.
The simulations are run in double precision with randomized domain centers.

Because small numerical differences in initial conditions can accumulate
over the course of nonlinear $N$-body evolution, we take preventative measures
to ensure strict consistency between the target data and our model.
During training, rather than regenerating the initial conditions,
we load the exact initial conditions used in the \GADGET\ simulations directly
into \pmwd, alongside the corresponding output snapshots.
...
other loss(es).
While the logarithmic loss is generally suitable for adaptive optimizers in
fixed simulation settings, training across diverse configurations simultaneously
can lead to scale imbalances.
We therefore normalize the residuals within the logarithm using the variance of
the target field.
...
and further average over all $n_{\rm snaps}$ snapshots in a single forward simulation
%
\begin{equation}
\cJ = \frac{1}{n_{\rm snaps}} \sum_{i=1}^{n_{\rm snaps}} \left[
                      \cL_{\vs(a_i)} + \cL_{\delta(a_i)}\right].
\end{equation}
%
...
The NNs are initialized to match the original PM simulation.
Specifically, we set the weights of the output layer to zero and its bias
to one, while the hidden layers use standard He initialization.
These settings ensure the initial output is exactly unity, corresponding
to the original PM simulation without spatial optimization.
The parameters are initialized to be identical across all devices, ensuring they
remain synchronized throughout training as they receive the same averaged
gradients.
...
distributed to the processes with each process bound to a single GPU
device.
...
initial conditions, cosmological parameters and other common
configurations as the target training data.
As discussed in \autoref{sec:loss}, the loss in a single simulation averages
over all snapshots.
We use a total batch size of 64 simulations.
With 16 GPU devices, this is achieved by accumulating gradients over 4 training
steps per device before each optimizer update.
The loss and gradients are averaged across all devices and accumulation steps.

We train the model using the AdamW \citep{AdamW} optimizer with a learning rate
of $10^{-4}$ and a weight decay of $0.01$.
...
We evaluate the optimized simulation by comparing its output against the
target \GADGET\ simulation at both the field and halo levels.

For the field-level comparison, we introduce for each Fourier mode two
quantities.
The transfer function (TF) is defined as the square root ratio of the
auto power spectra of two fields $\hat f$ and $f$:
%
\begin{equation}
T(\vk) \triangleq
...
\end{equation}
%
where the numerator is the real part of the cross product of two
corresponding Fourier modes.
Together, TF and CC provide a complete characterization of the
per-mode residual between two fields.
For each mode, the power of the residual field
$|\hat f(\vk) - f(\vk)|^2$ can be decomposed as
%
\begin{equation}
\frac{|\hat f - f|^2}{|f|^2}
= \bigl[T(\vk) - 1\bigr]^2 + 2\,T(\vk)\bigl[1 - r(\vk)\bigr],
\label{residual_decomp}
\end{equation}
%
where the first term captures the amplitude mismatch and the second
captures the phase or stochastic mismatch.
The residual vanishes if and only if $T=1$ and $r=1$ simultaneously,
meaning that the two fields are identical mode by mode.
Thus TF and CC are sufficient metrics for a field-level comparison:
any discrepancy in either amplitude or phase will be reflected by a
deviation in $T$ or $r$ from unity.
In practice, we average $T$ and $r$ within spherical $k$-shells to
obtain smooth functions of the wavenumber magnitude $k$.

While TF and CC capture the agreement of the continuous density field,
cosmological analyses also rely on the properties of collapsed
structures.
We therefore additionally assess the accuracy of the optimized
simulation at the level of dark matter halos.
Halos are identified using the \textsc{Rockstar} phase-space halo
finder \citep{Behroozi2013}, which locates overdensities in the
six-dimensional position--velocity space of particles.
The same \textsc{Rockstar} configuration is applied to both the target
\GADGET\ and the optimized PM simulations to ensure a consistent
comparison.
We then compare the halo mass function (HMF),
i.e.\ the comoving number density of halos per logarithmic mass interval
$\mathrm{d}n / \mathrm{d}\ln M$, between the two catalogs.
The ratio of the optimized HMF to the target HMF quantifies the
mass-dependent accuracy of the halo abundance.
...
\begin{figure*}
  \centering
  \input{training_pipeline}
  \caption{Schematic overview of the distributed training pipeline.
  The architecture spans persistent storage, host (CPU) orchestration, and
  distributed device (GPU) execution to maximize throughput for cosmological
  $N$-body workloads.
  The \GSDATA\ Sobol sequence targets are distributed across 16 CPU processes,
  which load initial conditions and target snapshots.
  An asynchronous queue (\texttt{PrefetchToDevice}) maintains a depth of two
  batches to prevent GPU idling during disk I/O.
  The core iteration loop leverages JAX's \texttt{pmap} to compile and execute
  the forward simulation, multi-snapshot loss evaluation, and adjoint
  backpropagation concurrently across devices.
  Local gradients are accumulated over multiple forward--backward passes and
  averaged globally via \texttt{lax.pmean} prior to an Optax optimizer step.
  Finally, parameters and optimizer states are checkpointed periodically to
  ensure fault tolerance.}
  \label{fig:pipeline}
\end{figure*}

Our training orchestration is engineered to efficiently handle the heavy
computational workload of cosmological simulations.
The full details of the distributed data loading, asynchronous prefetching, and
multi-device JAX execution are summarized in \autoref{fig:pipeline}.

...
def itp_prev(ptcl, a0, a1, a, cosmo):
    ...
    h00 = 2 * t**3 - 3 * t**2 + 1
    h10 = t**3 - 2 * t**2 + t
    disp = (h00.astype(dtype) * ptcl.disp +
            (h10 * Da / a3E0).astype(dtype) * ptcl.vel)
    # velocity
    # derivatives of the Hermite basis functions
    h00 = 6 * t**2 - 6 * t
    h10 = 3 * t**2 - 4 * t + 1
    vel = ((h00 / Da).astype(dtype) * ptcl.disp +
           (h10 / a3E0).astype(dtype) * ptcl.vel)
...
def itp_next(ptcl, a0, a1, a, cosmo):
    ...
    h01 = - 2 * t**3 + 3 * t**2
    h11 = t**3 - t**2
    disp = (h01.astype(dtype) * ptcl.disp +
            (h11 * Da / a3E1).astype(dtype) * ptcl.vel)
    # velocity
    # derivatives of the Hermite basis functions
    h01 = - 6 * t**2 + 6 * t
    h11 = 3 * t**2 - 2 * t
    vel = ((h01 / Da).astype(dtype) * ptcl.disp +
           (h11 / a3E1).astype(dtype) * ptcl.vel)
    vel *= (a**3 * jnp.sqrt(E2(a, cosmo))).astype(dtype)

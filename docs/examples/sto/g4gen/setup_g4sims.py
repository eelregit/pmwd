...
def gen_g4files(sim_dir, i,
                fn_sobol='sobol.txt',
                tpl_config='Config.sh',
                tpl_param='param.txt',
                tpl_job='job.sh'):
    """
    ...
    with (open(tpl_param, 'r') as f,
        ...
        param = f.read()
        param = param.format(a_start=conf.a_start,
                             a_stop=conf.a_stop,
                             box_size=conf.box_size[0],
                             M_g=conf.M*1e3*cosmo.h,  # TODO double check the units
                             L_cm=conf.L*1e2*cosmo.h,
                             V_cm_s=conf.V*1e5,
                             H_0=100.0,
                             G=0,
                             ...
                             h=cosmo.h,
                             soften_cmv=sobol[8],
                             soften_phy=sobol[8]*1.5,  # TODO check this
                            )
        fo.write(param)

    # times (scale factors) of output snapshots
    outtimes = np.linspace(1/16, 1, 121) + sobol[1]
    np.savetxt(os.path.join(sim_dir, 'outtimes.txt'), outtimes, fmt='%.15f')

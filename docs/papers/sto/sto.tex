...
2^{1+i/3} \pi / L$.
Our loss on a field $f$ sums up its MSE relative to the target field
power in logarithmic bins:
%
\begin{equation}
\cL_f = \frac1{N_K} \sum_i
\frac{\sum_{k \in K_i} w(\vk) \bigl| \hat f(\vk) - f(\vk) \bigr|^2}
...
%
\begin{equation}
\prod_{j=1}^3 \frac{k_j l / 2}{\tan(k_j l /2)} \textrm{and its square},
\prod_{j=1}^3 \cos (k_j l / 2) \textrm{and its square},
\end{equation}
%

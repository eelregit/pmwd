...
Our observable is a snapshot $\hat \vz = (\hat\vx, \hat\vp)^\intercal$ at any
given $a$, which in general can be given by the interpolation of two snapshots
$\vz_{k-1}$ and $\vz_k$.
Following the discussion in \citet{Li2022a}, the observe operator in the adjoint
equation time stepping from $a_i$ to $a_{i-1}$ now reads
\begin{align}
  O_i^{i-1}: \qquad \vxi_{i-1} :\:\!\!\!&= \vxi_{i-1}
  + \frac{\p\cJ}{\p\hat\vx} \cdot \frac{\p\hat\vx}{\p\vx_{i-1}} \,, \nonumber\\
                    \vpi_{i-1} :\:\!\!\!&= \vpi_{i-1}
  + \frac{\p\cJ}{\p\hat\vp} \cdot \frac{\p\hat\vp}{\p\vp_{i-1}} \,,
\end{align}
where $\vlambda = (\vxi, \vpi)$ are the adjoint variables of $\vz = (\vx,
\vp)^\intercal$, and ${\p\hat\vz}/{\p\vz_i}$ is zero except for the relevant
$\vz_{k-1}$ and $\vz_k$.
Similarly we have in the initial conditions of the adjoint equations
\begin{align}
  \vxi_n &= \frac{\p\cJ}{\p\hat\vx} \cdot \frac{\p\hat\vx}{\p\vx_n} \,, \nonumber\\
  \vpi_n &= \frac{\p\cJ}{\p\hat\vp} \cdot \frac{\p\hat\vp}{\p\vp_n} \,.
\end{align}
...
As derived in \citet{Li2022a}, the gradient of the objective function
$\cJ(\hat\vz(\vz_{k-1}, \vz_k, \vtheta), \theta)$ is accumulated through
\begin{multline}
...
We use a B-spline to model the function $a(t)$ that maps uniformly sampled $t$
values to optimally distributed $a$ time steps.
The spline is parameterized with a few control points that are updated during
training.

...
The dynamics of the dark matter is governed by the Poisson equation
%
\begin{equation}
...
\end{equation}
%
where the Laplacian operator reduces to simple multiplications.
...
solving the Poisson equation given numerous particles.
In order to use the fast Fourier transform (FFT), PM estimates the
matter density field on a regular spatial grid by scattering the
particles onto an auxiliary mesh.
...
The gravitational forces on the mesh points are computed in the Fourier
space and transformed back to the configuration space, which are then
grathered to form the interpolated force for each particle, with the
same weights as in the scattering process.
...
The PM method is mainly limited by its resolution on small scales, i.e.
scales around and below the mesh cell size.
...
regular discrete mesh.

In this work, we sharpen the gravitational force by embedding NNs in the
force equation and train them for the spatial optimization (SO) of the
PM simulation.

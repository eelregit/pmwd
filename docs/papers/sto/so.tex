...
\item (train, valid) Show $T - 1$ in symlog with 0.01 linthresh, and
  $1-r^2$ in log at 3 different $a$'s; Show $f$ and $g$ (along axis and
  face diagonal) at 3 different $a$'s; Track mean epoch loss
\item (train) Add random offset in loss grid (see \autoref{tab:param})
  to remove the position dependence in loss.
\item (train) Randomize snapshots of each simulation instead of
  sequential; alternatively accumulate their gradients
\item (train) 3 sets of grid: ptcl, force mesh, and loss grid. Explore
  if we can use a finer loss grid, from 1x to say 1.5x and even 2x.
\item (train, valid) Non-integer multiple mesh shapes: for uniform
...
\item (train) For generalizability and robustness, consider testing
  L1/L2 and dropout. Also they're worth adding if they give lower losses
  at convergence. Note that 512 realization are not too many, and we
  have features that can be very correlated.
...
\item (train) Reduce learning rate on (mean epoch) loss plateau in
  production
\item (valid) Interpolation test: compare SO DM and halo fields and
  summary stats with those of raw pmwd, deconvolved pmwd, and \GADGET,
  on the last 9 test cases and noncubic box.
\item (valid) Extrapolation test: demonstrate generalization capability,
...
%
\begin{equation}
\cJ = \ln\cL_\vs + \ln\cL_\vv + \ln\cL_\delta
...
components, so that $f g$ can be the most general way to modify the
force kernel.

...
% combination of differentiable simulations, NN, and SR
NNs are flexible and efficient universal function approximators
\citep{Hornik1989}, which can be used to learn physical processes
when embedded in numerical simulations.
NNs are typically trained with gradient based optimizers, which requires
the simulations to be differentiable.
...
The particle-mesh (PM) method is a fundamental technique for
cosmological $N$-body simulations.
PM solves the Poisson equation by scattering the particles onto an
auxiliary mesh, on which the fast Fourier transform (FFT) is performed
and the Laplacian operator reduces to a simple multiplication of
wavenumbers in Fourier space.
The cloud-in-cell (CIC) scheme \citep{HockneyEastwood1988} has been
widely used to compute the mass fractions of a particle at $\vx'$
scattered to a nearby mesh grid point at $\vx$
%
\begin{equation}
  W(\vx, \vx') = \prod_{i=1}^3
...
The gravitational forces on the mesh points are then grathered to form
the interpolated force for each particle, with the same weights as in
the scattering process.
Other scattering schemes include the nearest grid point (NGP) and
triangle shaped cloud (TSC) etc.
Although being computationally efficient, the PM method is limited by
its resolution on small scales, i.e. scales around and below the mesh
cell size.
This is due to the loss of information during the scattering and
gathering process between the continuously distributed particles and the
discrete mesh.
...
force by embedding NNs in the Poisson equation and train them for the
spatial optimization (SO) of the simulation resolution on small scales.
We further interpret the trained NNs using symbolic regression (SR).
...
In Fourier space, the gravitational force field $\vF = - \vnabla \varphi$
follows
%
\begin{equation}
\vF(\vk) = - i \vk \varphi(\vk) =
...
scattering the particles onto the mesh with e.g. the CIC scheme in
\eqref{cic}.
Then the force on each particle is estimated by gathering the force
field from the mesh.
...
sharpening by modifying the force equation \eqref{force} to
%
\begin{equation}
